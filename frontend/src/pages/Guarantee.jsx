    ...
    <div className="max-w-[1280px] min-h-screen mx-auto px-4 py-4">
      ...
      <div className="text-center py-12 border-b">
        ...
        <div className="max-w-2xl mx-auto mt-6">
          ...
          <p className="text-gray-600 futura">{t('guarantee.intro')}</p>
        </div>
      </div>
      ...
      <div className="my-12">
        <div className="bg-green-50 rounded-lg p-6">
          ...
          <p className="text-gray-600 futura">{t('guarantee.covered.description')}</p>
        </div>
      </div>
        ...
        <div className="bg-red-50 rounded-lg p-6">
          ...
          <ul className="space-y-2">
            {['mechanical', 'repair', 'misuse', 'chemical'].map((item) => (
              <li key={item} className="flex items-center gap-2 text-gray-600 futura">
      ...
      <div className="bg-yellow-50 rounded-lg p-6 my-12">
        ...
        <p className="text-gray-600 futura">{t('guarantee.important.description')}</p>
      </div>
      ...
      <div className="my-12">
        ...
        <p className="text-gray-600 futura mb-6">{t('guarantee.care.description')}</p>
        <ul className="space-y-4">
          {careRules.map((rule) => (
            <li key={rule.id} className="flex items-center gap-3 bg-gray-50 p-4 rounded-lg">
              ...
              <p className="text-gray-600 futura">{t(`guarantee.care.rules.${rule.id}`)}</p>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="bg-mainColor text-white rounded-lg p-8 my-12">
        <p className="text-center text-lg futura leading-relaxed">{t('guarantee.conclusion')}</p>
      </div>
    </div>
  );
};
